...
public class MainWindow extends JFrame {
    ...
    private JRadioButton rbOnsite, rbTakeAway;
    ...
    private JList<Product> foodList, drinkList;
    private DefaultListModel<Product> foodModel;
    private DefaultListModel<Product> drinksModel;
	...
	for (Product p : mcDonalds.getMenuProducts()) {
	    pnProducts
		    .add(newButton(mcDonalds.getMenuProducts().indexOf(p), p));
	}
    }
	...
	for (int i = 0; i < pnProducts.getComponents().length; i++) {
	    ...
	    adaptImage(boton,
		    mcDonalds.getMenuProducts().get(i).getImagePath());
	}
    }
	...
	Product a = mcDonalds.getMenuProducts().get(position);

	// There is no spinner so it is always point to be 1 unit, 1 per click
	mcDonalds.addToOrder(a, 1);

	// Show it in the list
	showInList(mcDonalds.getMenuProducts().get(position));

	// Update the price
	getTxtPrice().setText(
		"Price: " + String.format("%.2f", mcDonalds.getOrderTotal()));
	...
	// It expects the contents and the name of the panel you want
	...
	if (checkFields()) {
	    ...
	    ((CardLayout) getPnContents().getLayout()).show(getPnContents(),
		    "pn3");
	}
    }
	...
	return (txtName.getText().equals("")
		|| (String.valueOf(psw1.getPassword()).equals(""))
		|| (String.valueOf(psw2.getPassword()).equals("")));

    }

    private boolean isWrong() {
	return (!String.valueOf(psw1.getPassword())
		.equals(String.valueOf(psw2.getPassword())));
    }
	...
	if (pnOrderData == null) {
	    ...
	    pnOrderData.setBorder(
		    new TitledBorder(UIManager.getBorder("TitledBorder.border"),
			    "Order Information", TitledBorder.LEADING,
	...
	if (pnInfo2 == null) {
	    ...
	    // CANNOT DO THIS --> pnInfo2.add(getPnOrder(),
	    // BorderLayout.CENTER);
	}
	return pnInfo2;
    }
	...
	if (lbOk == null) {
	    lbOk = new JLabel("");
	    lbOk.setIcon(
		    new ImageIcon(MainWindow.class.getResource("/img/ok.png")));
	...
	if (pnOrder == null) {
	    ...
	    // Adding the tabbed panes and including mnemonics
	    pnOrder.addTab("Food", null, getScFood(), null);
	    pnOrder.setMnemonicAt(0, KeyEvent.VK_F);
	    pnOrder.addTab("Drinks", null, getScDrinks(), null);
	    pnOrder.setMnemonicAt(1, KeyEvent.VK_D);
	}
	return pnOrder;
    }
	...
	if (btnNext1 == null) {
	    ...
	    btnNext1.setEnabled(false);
	    btnNext1.setFont(new Font("Tahoma", Font.BOLD, 16));
	    btnNext1.setBackground(Color.GREEN);
	}
	return btnNext1;
    }
	...
	if (btnPrevious2 == null) {
	    ...
	    btnPrevious2.setBackground(Color.GREEN);
	    btnPrevious2.setFont(new Font("Tahoma", Font.BOLD, 16));
	}
	return btnPrevious2;
    }
	...
	if (btnNext2 == null) {
	    ...
	    btnNext2.setBackground(Color.GREEN);
	    btnNext2.setFont(new Font("Tahoma", Font.BOLD, 16));
	}
	return btnNext2;
    }
	...
	if (btnPrevious3 == null) {
	    ...
	    btnPrevious3.setBackground(Color.GREEN);
	    btnPrevious3.setFont(new Font("Tahoma", Font.BOLD, 16));
	}
	return btnPrevious3;
    }
	...
	if (scDrinks == null) {
	    ...
	    scDrinks.setViewportView(getDrinkList());

	}
	return scDrinks;
    }

    private JList<Product> getDrinkList() {
	if (drinkList == null) {
	    drinkList = new JList<Product>();
	    drinksModel = new DefaultListModel<Product>();
	    drinkList.setModel(drinksModel);
	}
	return drinkList;
    }
}
